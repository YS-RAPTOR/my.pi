...
const rewritePrompt = (text: string, resolved: ReadonlyArray<ResolvedReference>) => {
  let output = "";
  let cursor = 0;
  ...
  Effect.gen(function* () {
    ...
    yield* barriers.handle(
      ...
      Effect.fn("Features.BetterSkills.Inline.sessionStart")(function* () {
        ...
        const skillItems = Effect.fn("Features.BetterSkills.Inline.__skillItems")(function* (
          query: string,
        ) {
          ...
          const matches =
            query === "" ? available : fuzzyFilter(available, query, ({ skill }) => skill.name);
        ...
        const provider = (current: AutocompleteProvider): AutocompleteProvider => ({
          triggerCharacters: [...new Set([...(current.triggerCharacters ?? []), "$"])],
          async getSuggestions(lines, cursorLine, cursorColumn, options) {
            const delegate = () => current.getSuggestions(lines, cursorLine, cursorColumn, options);
          ...
          Effect.gen(function* () {
            ...
            if (unavailable !== undefined) {
              ...
              yield* callback.ui.notify(
                decision.reason ?? `Skill ${unavailable.skill.name} is unavailable`,
                "warning",
              );
              return { action: "handled" as const };
            }

            const skills = new Map(resolved.map(({ skill }) => [skill.filePath, skill])).values();
            const blocks = yield* Effect.forEach(skills, (skill) => runtime.render(skill, cwd));
            const prompt = rewritePrompt(event.text, resolved);
            const result = {
              action: "transform" as const,
              text: prompt === "" ? blocks.join("\n\n") : `${blocks.join("\n\n")}\n\n${prompt}`,
            };
