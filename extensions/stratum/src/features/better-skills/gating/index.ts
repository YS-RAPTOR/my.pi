...
const explicitSkillNames = (text: string, inline: boolean) => {
  ...
  return inline ? Runtime.findInlineReferences(text).map(({ name }) => name) : [];
};
...
const overview = (snapshot: Catalog.Snapshot) => {
  ...
  for (const [state, label] of states) {
    const decisions = snapshot.decisions.filter((decision) => decision.state === state);
    ...
    for (const decision of decisions) {
      ...
      const condition = state === "unavailable" ? decision.availability : decision.modelInvocation;
      const reason = conditionReason(condition)
        .trim()
        .split(/\s*\n+\s*/)
        .join(" ");
      sections.push(`    ${state === "unavailable" ? "available-if " : ""}${reason}`);
    }
  }
...
const explain = (decision: Catalog.Decision) => {
  const formatCondition = (label: string, condition: Catalog.Condition) => {
    ...
    if (output !== undefined) {
      ...
      lines.push(
        `  output: ${JSON.stringify(trimmed.length > 500 ? `${trimmed.slice(0, 500)}…` : trimmed)}`,
      );
    }
...
const promptSkills = (skills: ReadonlyArray<Pick<Skill, "name" | "filePath">>) =>
...
export const layer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    const evaluateCommands = (cwd: string) =>
      Effect.flatMap(commandSkills(), (skills) => catalog.evaluate({ cwd, skills }));
    ...
    yield* barriers.handle(
      ...
      Effect.fn("Features.BetterSkills.Gating.sessionStart")(function* () {
        ...
        const context = yield* Effect.context<Pi.Host.Service | Pi.Host.Callback>();
        ...
        yield* callback.ui.addAutocompleteProvider((current) => ({
          ...
          async getSuggestions(lines, line, column, options) {
            ...
            if (!suggestions.items.some(({ value }) => commandSkillName(value))) {
              return suggestions;
            }
            try {
              ...
              const items = suggestions.items.filter(({ value }) => {
                ...
                return name === undefined || findDecision(snapshot, name)?.state !== "unavailable";
              });
    ...
    yield* interceptors.handle(
      "before_agent_start",
      0,
      Effect.fn("Features.BetterSkills.Gating.beforeAgentStart")(function* (event) {
        const callback = yield* Pi.Host.Callback;
        ...
        for (const skill of skills) {
          ...
          visible.push({
            ...skill,
            disableModelInvocation: skill.disableModelInvocation || decision?.state === "user-only",
          });
        }
        visible.sort((left, right) => {
          const byName = left.name.localeCompare(right.name);
          return byName === 0 ? left.filePath.localeCompare(right.filePath) : byName;
        });
    ...
    yield* interceptors.handle(
      ...
      Effect.fn("Features.BetterSkills.Gating.toolCall")(function* (event) {
        ...
        if (Option.isNone(decision) || decision.value.state === "model-accessible") {
          return;
        }
    ...
    yield* contributions.command("skills", {
      ...
      getArgumentCompletions: Effect.fn("Features.BetterSkills.Gating.skills.completions")(
        function* (prefix) {
      ...
      handler: Effect.fn("Features.BetterSkills.Gating.skills")(function* (args) {
        const callback = yield* Pi.Host.Callback;
        const command = yield* Pi.Host.Command;
        const cwd = yield* callback.session.cwd;
        const skills = promptSkills((yield* command.agent.getSystemPromptOptions).skills ?? []);
        const [subcommand, name] = args.trim().split(/\s+/, 2);

        switch (subcommand) {
          case "":
            yield* callback.ui.notify(overview(yield* catalog.evaluate({ cwd, skills })), "info");
            return;

          case "reload": {
            const snapshot = yield* catalog.reload({ cwd, skills });
            yield* callback.ui.notify(`Reloaded ${snapshot.decisions.length} skills.`, "info");
            return;
          }

          case "explain": {
            if (name === undefined) {
              yield* callback.ui.notify("Usage: /skills explain <name>", "warning");
              return;
            }
            const decision = findDecision(yield* catalog.evaluate({ cwd, skills }), name);
            yield* callback.ui.notify(
              decision === undefined ? `Unknown skill: ${name}` : explain(decision),
